...
if page == "🏠 Dashboard":
    ...
    with col1:
        ...
        fig = px.box(df, x="city", y="nightly_price_usd", color="city",
                     ...
                     labels={"nightly_price_usd":"Nightly Price (USD)","city":""})
    ...
    for city, col in zip(["Dubai","Goa","Maldives"],[col3,col4,col5]):
        ...
        col.plotly_chart(fig3, width='stretch')
    st.markdown('</div>', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# PAGE: MARKET MAP  ← FIXED: scatter_mapbox → scatter_map
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    # FIXED: px.scatter_mapbox → px.scatter_map  |  mapbox_style → map_style
    fig = px.scatter_map(map_df, lat="latitude", lon="longitude",
        color="investment_score", size="nightly_price_usd", hover_name="id",
        hover_data={"area":True,"property_type":True,"nightly_price_usd":True,
                    "roi_pct":True,"investment_label":True,"latitude":False,"longitude":False},
        color_continuous_scale=[[0,"#e84040"],[0.5,"#e8a216"],[1,"#0db870"]],
        map_style="carto-darkmatter", zoom=3, height=560,
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        fig = px.scatter(df, x=col_x, y="nightly_price_usd", color="city", trendline="ols",
                         color_discrete_map=CITY_COLORS, opacity=0.55,
                         labels={"nightly_price_usd":"Nightly Price (USD)"})
    ...
    with tab3:
        ...
        fig3 = px.bar(occ_data, x="property_type", y="occupancy_rate", color="city", barmode="group",
                      color_discrete_map=CITY_COLORS,
                      labels={"occupancy_rate":"Avg Occupancy Rate","property_type":"Property Type"})
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        arch_data = {
            "Layer": ["Data collection","Feature engineering","Price prediction","ROI classification","Investment scoring","AI chatbot","Dashboard","Deployment"],
